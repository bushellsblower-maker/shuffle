...
import { MAX_SPEED, World, speedForDistance } from "./physics.ts";
...
/** What a browser does: step the same world with whatever frame times it gets. */
function replayLikeBrowser(table: readonly TableWeight[], shot: ShotInput, id: number, team: Team, rand: () => number): TableWeight[] {
...
test("sync invariant: a browser replay at any frame rate lands exactly where the room does", () => {
  const rand = seeded(42);
  for (let game = 0; game < 6; game++) {
    let table: TableWeight[] = [];
    for (let id = 0; id < SHOTS_PER_ROUND; id++) {
      const team = (id % 2) as Team;
      const shot = randomShot(rand);
      const room = simulateShot(table, shot, id, team);
      const browser = replayLikeBrowser(table, shot, id, team, rand);
      assert.deepEqual(browser, room, `game ${game} shot ${id}`);
      table = room;
    }
  }
});
